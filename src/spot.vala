...
public string band_from_khz(int khz) {
    ...
    if (mhz >= 1.8   && mhz < 2.0)    return RadioConstants.BANDS[1];
    if (mhz >= 3.5   && mhz < 4.1)    return RadioConstants.BANDS[2];
    if (mhz >= 5.25  && mhz < 5.45)   return RadioConstants.BANDS[3];
    if (mhz >= 7.0   && mhz < 7.3)    return RadioConstants.BANDS[4];
    if (mhz >= 10.1  && mhz < 10.15)  return RadioConstants.BANDS[5];
    if (mhz >= 14.0  && mhz < 14.35)  return RadioConstants.BANDS[6];
    if (mhz >= 18.068&& mhz < 18.168) return RadioConstants.BANDS[7];
    if (mhz >= 21.0  && mhz < 21.45)  return RadioConstants.BANDS[8];
    if (mhz >= 24.89 && mhz < 24.99)  return RadioConstants.BANDS[9];
    if (mhz >= 28.0  && mhz < 29.7)   return RadioConstants.BANDS[10];
    if (mhz >= 50.0  && mhz < 54.0)   return RadioConstants.BANDS[11];
    if (mhz >= 144.0 && mhz < 148.0)  return RadioConstants.BANDS[12];
    if (mhz >= 420.0 && mhz < 450.0)  return RadioConstants.BANDS[13];
...
public sealed class Spot : Object {
  ...
  public Spot.from_json (Json.Object spot) {
    Object(
      ...
      frequency_khz: int.parse (spot.get_string_member_with_default ("frequency", "0")),
      spot_time: new GLib.DateTime.from_iso8601 (spot.get_string_member ("spotTime"), new GLib.TimeZone.utc ())
    );
  }
  ...
  public string to_string() {
    return @"Spot(activator: $callsign\nspotter: $spotter\npark: $park_ref\nfrequency: $frequency_khz)";
  }
}
