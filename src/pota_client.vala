...
public sealed class PotaClient : Object {
    ...
    public async void post_spot (Spot spot) throws Error {
        ...
        if (message.get_status () != Soup.Status.OK) {
            warning ("POST failed: %u %s", message.get_status (), message.get_reason_phrase ());
            return;
        }
    }

    public async Json.Node? fetch_spot_history (string callsign, string park_ref) throws Error {
        ...
        var url = POTA_BASE_URL + "/v1/spots/" + escaped_callsign + "/" + escaped_park_ref;

        return yield fetch_worker (url);
    }

    public async Json.Node? fetch_operator (string callsign) throws Error {
        var url = POTA_BASE_URL + "/stats/user/" + GLib.Uri.escape_string (callsign, null, false);
        return yield fetch_worker (url);
    }
