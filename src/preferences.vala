...
public sealed class PreferencesDialog : Object {
  ...
  private void setup_bindings() {
    settings.bind("callsign", row_callsign, "text", SettingsBindFlags.DEFAULT);
    settings.bind("location", row_location, "text", SettingsBindFlags.DEFAULT);
    settings.bind("spot-message", row_spot_message, "text", SettingsBindFlags.DEFAULT);
    settings.bind("update-interval", row_update_interval, "value", SettingsBindFlags.DEFAULT);

    bind_combo_to_string_setting("default-band", row_default_band);
    bind_combo_to_string_setting("default-mode", row_default_mode);
    bind_combo_to_string_setting("radio-connection-type", row_connection_type);

    settings.bind("radio-model", row_radio_model, "selected", SettingsBindFlags.DEFAULT);
    settings.bind("radio-device", row_device_path, "text", SettingsBindFlags.DEFAULT);
    bind_baud_rate_combo();
    settings.bind("radio-network-host", row_network_host, "text", SettingsBindFlags.DEFAULT);
    settings.bind("radio-network-port", row_network_port, "value", SettingsBindFlags.DEFAULT);

    settings.bind("enable-logging", row_enable_logging, "active", SettingsBindFlags.DEFAULT);
    settings.bind("qrz-api-key", row_qrz_api_key, "text", SettingsBindFlags.DEFAULT);
    settings.bind("highlight-unhunted-parks", row_highlight_unhunted, "active", SettingsBindFlags.DEFAULT);
  }
  ...
  private void on_import_file() {
    ...
    file_dialog.open.begin(dialog.get_parent() as Gtk.Window, null, (obj, res) => {
      try {
